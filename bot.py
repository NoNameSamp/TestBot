...
# ID для кликабельного пинга в чате. Зашит намертво — бот всегда пингует
# только Вику (id 797490717); через переменную окружения или /admin
# поменять нельзя.
TARGET_ID = 797490717
    ...
    try:
        with open(STATE_FILE, "r", encoding="utf-8") as f:
            ...
            return {
                ...
                # По умолчанию (нет файла состояния / нет ключа) бот НЕактивен —
                # включить его нужно явно через /admin.
...
def get_bot_active() -> bool:
    return _state.get("bot_active", False)
...
# ---------- Отслеживание последнего сообщения Вики (чтобы отвечать на него) ----------
# Храним только в памяти (не в файле состояния): id последнего сообщения
# сбрасывается с каждым перезапуском бота, это ожидаемо — reply на старое
# сообщение после рестарта не критичен, бот просто отправит обычное сообщение.
_last_target_message = {"chat_id": None, "message_id": None}


async def track_target_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Запоминает id последнего сообщения от Вики в чате, чтобы поздравления
    могли быть отправлены как ответ (reply) на него."""
    message = update.effective_message
    if message is None:
        return
    _last_target_message["chat_id"] = message.chat_id
    _last_target_message["message_id"] = message.message_id


def get_target_reply_id(chat_id):
    """Возвращает message_id последнего сообщения Вики в данном чате (для
    reply_to_message_id), если оно известно и относится к этому же чату,
    иначе None — тогда сообщение отправится как обычное, без ответа."""
    if (
        _last_target_message["message_id"] is not None
        and str(_last_target_message["chat_id"]) == str(chat_id)
    ):
        return _last_target_message["message_id"]
    return None
...
async def safe_send_to_chat(context: ContextTypes.DEFAULT_TYPE, text: str, **kwargs):
    ...
    try:
        ...
        # Сообщение, на которое пытались ответить (reply_to_message_id), могло
        # быть удалено или недоступно — отправляем обычным сообщением, без reply.
        if "reply_to_message_id" in kwargs and "message to be replied not found" in str(e).lower():
            logger.warning("Сообщение Вики для ответа не найдено, отправляю без reply")
...
async def on_bot_added_to_chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    По умолчанию бот молчит: ничего не пишет в чат при подключении, только
    фиксирует событие в логах. Включать бота нужно явно через /admin."""
    ...
    if just_added:
        logger.info("Бот добавлен в чат %s, приветствие не отправляется (тихий режим).", result.chat.id)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Не-админ. По умолчанию бот выключен (get_bot_active() == False) и не
    # реагирует вообще ни на что, даже на /start — полная тишина. Ответ
    # "Бот работает" в ЛС появляется только после того, как админ включит
    # бота через /admin.
    if not get_bot_active():
        return
    if chat is not None and chat.type == "private":
        # В ЛС при включённом боте отвечаем только на /start и только этим
        # текстом, никакой другой функциональности не-админам в ЛС не показываем.
...
async def on_button_pressed(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reply_id = get_target_reply_id(update.effective_chat.id)
    try:
        ...
        # Сообщение Вики, на которое пытались ответить, могло быть удалено.
        if reply_id is not None and "message to be replied not found" in str(e).lower():
            logger.warning("Сообщение Вики для ответа не найдено, отправляю без reply")
            await update.message.reply_text(
...
async def scheduled_greeting(context: ContextTypes.DEFAULT_TYPE):
    if not get_bot_active():
        # Бот выключен (в т.ч. по умолчанию, пока админ явно не включит) —
        # автопоздравления по расписанию тоже молчат.
        return
    greeting = await generate_greeting(get_target_name())
    text = f"{mention_or_name()}\n\n{greeting}"
    reply_id = get_target_reply_id(CHAT_ID)
...
async def post_init(application: Application):
    # По умолчанию бот молчит при старте/рестарте: никаких сообщений в чат
    # не отправляется. Включение и показ кнопки — только через явную команду
    # администратора /admin.
    logger.info("Бот запущен (тихий режим, ничего не отправляется в чат).")
...
async def toggle_bot_active(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Никакой рассылки в общий чат — ни при включении, ни при выключении.
    # Ответ виден только администратору, который вызвал команду.
    await update.message.reply_text(
...
def main():
    ...
    # Отдельная группа (group=1), чтобы этот хендлер работал параллельно с
    # остальными и не "перехватывал" обработку сообщения — только запоминает
    # id последнего сообщения Вики, чтобы потом ответить на него поздравлением.
    if TARGET_ID is not None:
        application.add_handler(
            MessageHandler(filters.User(user_id=TARGET_ID), track_target_message),
            group=1,
        )
