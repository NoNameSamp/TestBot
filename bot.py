...
async def publish_ad(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str, photo_file_id: str | None):
    ...
    author_name = html.escape(author.full_name)
    author_link = f'<a href="tg://user?id={author.id}">{author_name}</a>'
    ...
    if photo_file_id:
        await context.bot.send_photo(chat_id=target_chat, photo=photo_file_id, caption=caption, parse_mode="HTML")
    else:
        await context.bot.send_message(chat_id=target_chat, text=caption, parse_mode="HTML")
