...
DEFAULT_BIRTHDAY_NAME = os.environ.get("BIRTHDAY_NAME", "Пользователь")
...
async def admin_flow_got_id(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        "Принято. Теперь напишите имя именинника в именительном падеже "
        f'(отвечает на вопрос "кто?", например: {DEFAULT_BIRTHDAY_NAME}, Вика). '
        "В кнопке оно склонится в винительный падеж автоматически "
        "(Вика → Вику), а в тексте поздравления останется как есть.\n"
