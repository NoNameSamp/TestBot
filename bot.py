...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if _is_admin(user.id, admin_ids):
        text = (
            ...
            "/remove_button — убрать клавиатуру с кнопкой\n"
            "/restore_button — вернуть клавиатуру\n"
...
async def on_congratulate_pressed(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    await update.message.reply_text(f"🎂 {user.first_name} поздравил(а) Татьяну с Днём Рождения!")
...
def main():
    ...
    application.add_handler(MessageHandler(filters.Text([BUTTON_TEXT]), on_congratulate_pressed))
